...
const isRealImage = (u) => {
  ...
  if (!s) return false;
  if (s.includes("/img/no-photo.png")) return false; // заглушку не считаем фото
  return true;
};

const hasImages = (p) =>
  Array.isArray(p.images) && p.images.some((x) => isRealImage(x));
...
const formatQtyForUI = (qty, measureName) => {
  const n = normNumber(qty);
  if (n == null) return null;

  const m = String(measureName || "").trim().toLowerCase();
...
function normalizeProduct(p) {
  const images = Array.isArray(p.images) ? p.images.filter(isRealImage) : [];

  const measureName = normText(p.measureName ?? p.measure_name);
  const qtyValue = normQty(p.quantity_value ?? p.quantity);
...
const qtyDisplay = formatQtyForUI(qtyValue, measureName);
      ...
      return {
        ...
        brand: normText(full?.brand) ?? normText(hit.brand) ?? null,
        price: full?.price ?? hit.price,
        barcode: normText(full?.barcode) ?? normText(hit.barcode) ?? null,

        images: full && hasImages(full) ? full.images : fromHit,

        // ✅ одна правда по остатку
