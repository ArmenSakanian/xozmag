<template>
  <div class="catalog-page">
    <section class="catalog-content">
      <div class="catalog-top">
        <div class="catalog-search">
          <HomeSearch :show-category="true" :current-category="currentCategory" :sync-route="true" route-key="q"
            catalog-path="/catalog" :server-limit="30" :dropdown-limit="12" @search-hits="searchHits = $event"
            @categories-loaded="onCategoriesLoaded" @categories-loading="catsLoading = $event" />
        ...
        <div v-if="hasActiveCategory && !isMobile" class="filters-bar">
          ...
          <div v-if="isRootCategorySelected === true" class="filter-block filter-type"
            ...
            <div class="filter-dropdown">
              ...
              <div v-show="openFilters.type" class="filter-dropdown-body"
                :class="{ scrollable: (typeOptions.length || 0) > 6 }">
                <label class="filter-checkbox filter-all">
                  <input type="checkbox" :checked="!typeModel.length" @change="
                    typeModel = [];
                  applyFilters();
                  " />
                  <span>Все</span>
                ...
                <label v-for="c in typeOptions" :key="c.id" class="filter-checkbox typeopt"
                  :class="{ parent: c.hasChildren }" :style="{ '--indent': (c.depth - 1) * 14 + 'px' }">
                  <input type="checkbox" :value="String(c.code)" v-model="typeModel" @change="applyFilters" />
            ...
            <div class="filter-dropdown">
              <div class="filter-dropdown-head" @click="toggleFilter('brand')">
                <span class="filter-head-text">
                  {{
                    brandModel.length
                    ? brandModel.length <= 2 ? brandModel.join("· ")
                      : `Выбрано: ${brandModel.length}`
                    : " Все" }} </span>
                    <span class="arrow" :class="{ open: openFilters.brand }">▾</span>
              </div>

              <div v-show="openFilters.brand" class="filter-dropdown-body" :class="{ scrollable: brands.length > 6 }">
                <label class="filter-checkbox filter-all">
                  <input type="checkbox" :checked="!brandModel.length" @change="
                    brandModel = [];
                  applyFilters();
                  " />
                  <span>Все</span>
          ...
          <div class="filter-block filter-photo" :class="{ open: openFilters.photo }">
      ...
      <div class="catalog-products">
        ...
        <template v-else>
          <div v-if="!hasActiveCategory && !searchQ" class="categories-landing">
            <HomeCatalogEntry :show-head="false" :items="topCats" :navigate-on-pick="false"
              @select-category="pickCategoryFromGrid" />
          </div>

          <div v-else class="products-grid">
            <article v-for="p in visibleProducts" :key="p.id" class="product-card">
              <div class="product-image" @click.stop @pointerdown.stop @mousedown.stop @touchstart.stop>
    ...
    <div v-if="showMobileFilters" class="moverlay-overlay" @click.self="
    ...
    ">
      <div class="moverlay-panel">
        <div class="moverlay-header">
          <button class="moverlay-btn moverlay-back" :class="{ ghost: mobileView === 'root' }"
            :disabled="mobileView === 'root'" @click="mobileView = 'root'" title="Назад">
          ...
          <button class="moverlay-btn moverlay-close" @click="
          showMobileFilters = false;
          mobileView = 'root';
          activeMobileAttr = null;
          " title="Закрыть">
        ...
        <div class="moverlay-body">
          ...
          <div v-if="mobileView === 'root'" class="mfil-list">
            <div v-if="isRootCategorySelected === true" class="mfil-item" @click="mobileView = 'type'">
              Тип товара
            ...
            <template v-if="allowAttrFilters">
              <div v-for="(vals, attr) in attributeFilters" :key="attr" class="mfil-item" @click="
          ...
          <div v-if="mobileView === 'type'" class="mfil-values">
            ...
            <label v-for="c in typeOptions" :key="c.id" class="filter-checkbox typeopt" :class="{ parent: c.hasChildren }"
          ...
          <div v-if="mobileView === 'attr'" class="mfil-values">
            ...
            <label v-for="v in attributeFilters[activeMobileAttr]?.values || []" :key="v.value" class="filter-checkbox">
              <input type="checkbox" :value="v.value" v-model="attributeModels[activeMobileAttr]"
                @change="applyFilters" />

              <span class="filter-option">
                <span v-if="attributeFilters[activeMobileAttr]?.ui_render === 'color'
                  " class="color-dot" :class="{ empty: !v.meta?.color }"
        ...
        <div class="moverlay-footer">
          <button class="moverlay-main" @click="
            showMobileFilters = false;
          mobileView = 'root';
          activeMobileAttr = null;
          ">
...
async function ensureProductsOnlyWhenCategory({ reset = false } = {}) {
  ...
  productsPromise = fetch(url, { cache: "no-store" })
    .then((r) => r.json())
    .then((json) => {
      // новый формат: {items, has_more, ...}
      const list = Array.isArray(json) ? json : (json.items || json.products || []);
      const chunk = (list || []).filter(Boolean).map(normalizeProduct);

      // доклеиваем без дублей по id
      const seen = new Set(products.value.map((x) => String(x.id)));
      ...
      serverOffset.value += chunk.length;

      // ✅ правильно: берем has_more если есть
      if (json && typeof json.has_more === "boolean") {
        serverHasMore.value = json.has_more;
      } else {
        // fallback если вдруг вернулся массив
        serverHasMore.value = chunk.length === serverLimit;
      }
